...
export const NewTaskLayout: React.FC = () => {
  ...
  // 👇 Category အကုန်ဆွဲဖို့ (projectId မပါ)
  const { 
    data: categories = [], 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('🚀 Submitting task with categoryId:', form.categoryId);
      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          <Stack spacing={2.5}>
            ...
            {/* Category Selection - အကုန်ပေါ်အောင်ပြင်ထားတယ် */}
            <FormControl fullWidth>
              <InputLabel id="category-select-label">Category (Optional)</InputLabel>
              <Select
                labelId="category-select-label"
