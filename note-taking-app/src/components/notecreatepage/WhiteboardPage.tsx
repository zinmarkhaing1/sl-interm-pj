...
//   // Canvas context ရဲ့ style တွေကို default configuration သတ်မှတ်ရန်
...
//   // component စတက်လာချိန် သို့မဟုတ် description ပြောင်းလဲချိန်မှာ ပုံဟောင်းရှိရင် ပြန်ဆွဲပေးရန်
//   useEffect(() => {
...
//       img.src = description;
//       img.onload = () => {
//         // ပုံအသစ်မဆွဲခင် ရှင်းလင်းပေးခြင်း (ပုံဟောင်းများ ထပ်မနေစေရန်)
//         ctx.clearRect(0, 0, canvas.width, canvas.height);
//         ctx.drawImage(img, 0, 0);
//         // image ဆွဲပြီးရင် context style တွေပျက်သွားနိုင်လို့ ပြန် config လုပ်ပေးခြင်း
//         configureContext(ctx);
//       };
//     }
//   }, [description]);
...
//     ctx.clearRect(0, 0, canvas.width, canvas.height);
//     setText(""); // ဘုတ်ပြားရှင်းလိုက်ရင် Parent state ကိုပါ blank ဖြစ်အောင်လုပ်ပေးခြင်း
//   };
...
//       <Button variant="outlined" color="error" onClick={clearCanvas} sx={{ mb: 2 }}>
//         Clear Board
//       </Button>
//       <Box sx={{ border: '2px dashed #ccc', display: 'inline-block', borderRadius: 2, overflow: 'hidden' }}>
//         <canvas
//           ref={canvasRef}
...
//   // Custom Styles & Tools States
//   const [color, setColor] = useState<string>('#000000');
//   const [lineWidth, setLineWidth] = useState<number>(3);
//   const [tool, setTool] = useState<string>('pencil'); // pencil, eraser, hand
...
//   // Sync canvas with description on load
//   useEffect(() => {
//     const canvas = canvasRef.current;
//     if (!canvas) return;
//     const ctx = canvas.getContext('2d');
...
//           display: 'flex', alignItems: 'center', gap: 0.5, p: 0.5, borderRadius: '6px'
//         }}
//       >
//         <IconButton size="small"><UndoIcon sx={{ fontSize: 18 }} /></IconButton>
//         <IconButton size="small"><RedoIcon sx={{ fontSize: 18 }} /></IconButton>
...
//             <Box
//               key={c}
//               onClick={() => { if (tool === 'eraser') setTool('pencil'); setColor(c); }}
//               sx={{
...
//           fullWidth
//         >
//           <ToggleButton value="S" sx={{ py: 0, fontSize: '11px', fontWeight: 'bold' }}>S</ToggleButton>
//           <ToggleButton value="M" sx={{ py: 0, fontSize: '11px', fontWeight: 'bold' }}>M</ToggleButton>
//           <ToggleButton value="L" sx={{ py: 0, fontSize: '11px', fontWeight: 'bold' }}>L</ToggleButton>
//           <ToggleButton value="XL" sx={{ py: 0, fontSize: '11px', fontWeight: 'bold' }}>XL</ToggleButton>
//         </ToggleButtonGroup>
//       </Paper>
...
//           sx={{ backgroundColor: tool === 'pencil' ? 'rgba(26, 115, 232, 0.1)' : 'transparent' }}
//         >
...
//           sx={{ backgroundColor: tool === 'eraser' ? 'rgba(26, 115, 232, 0.1)' : 'transparent' }}
//         >
//           <DeleteIcon fontSize="small" />
//         </IconButton>

//         <IconButton><TextFieldsIcon fontSize="small" /></IconButton>
//       </Paper>
...
//     // အကယ်၍ Text Tool ဖြစ်နေလျှင် စာရိုက်ရန် Input Box ကို ပေါ်စေမည်
//     if (tool === 'text') {
//       // စာရိုက်ပြီးသား ရှိနေရင် အရင်သိမ်းမယ်
//       if (textInputStyle.display === 'block' && textInputValue.trim()) {
//         finalizeText();
//       }

//       // Input box ၏ တည်နေရာကို တွက်ချက်ခြင်း
//       setTextInputStyle({
//         left: clientX,
//         top: clientY,
//         display: 'block'
...
//           color={tool === 'pencil' ? 'primary' : 'default'} 
//           onClick={() => setTool('pencil')}
//           sx={{ backgroundColor: tool === 'pencil' ? 'background.default' : 'transparent' }}
//         >
//           <CreateIcon fontSize="small" />
//         </IconButton>
...
//           color={tool === 'eraser' ? 'primary' : 'default'} 
//           onClick={() => setTool('eraser')}
//           sx={{ backgroundColor: tool === 'eraser' ? 'baackground.default' : 'transparent' }}
//         >
//           <DeleteIcon fontSize="small" />
//         </IconButton>

//         {/* Text Tool Active Button */}
//         <IconButton 
//           color={tool === 'text' ? 'primary' : 'default'} 
//           onClick={() => setTool('text')}
//           sx={{ backgroundColor: tool === 'text' ? 'background.default' : 'transparent' }}
//         >
...
export function WhiteboardPage({ description, setText }: WhiteboardPageProps) {
  ...
  // Infinite Loop ကာကွယ်ရန် Refs သတ်မှတ်ခြင်း
  ...
  // Canvas ကို Initial load နှင့် Tool/Color ပြောင်းချိန်များအတွက် စနစ်တကျ ထိန်းညှိခြင်း
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const ctx = canvas.getContext('2d');
    ...
    // description တကယ်ပြောင်းမှသာ Image ကို ပြန်ဆွဲမည် (Tool/Color ပြောင်းတိုင်း Clear မဖြစ်စေရန်)
  ...
  const saveCanvasData = () => {
    ...
    if (canvas) {
      const dataUrl = canvas.toDataURL();
      lastLoadedDescription.current = dataUrl; // ကိုယ်တိုင်ဆွဲလို့ပြောင်းလဲသွားတဲ့ data ကို Loop မပတ်အောင် မှတ်သားထားမည်
      setText(dataUrl);
    }
  };
