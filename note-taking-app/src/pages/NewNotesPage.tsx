...
export const NewNotePage: React.FC = () => {
  ...
  const getTaskProjectName = (task: NonNullable<typeof tasks>[number] | undefined) => {
    if (!task) return '';
    if (typeof task.project === 'object' && task.project !== null && 'name' in task.project) {
  ...
  useEffect(() => {
    ...
    setForm((prev) => ({
      ...
      endDate: selectedTask.dueDate ? selectedTask.dueDate.slice(0, 10) : prev.endDate,
    }));
  }, [selectedTask, form.taskId]);
      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          <Stack spacing={2.5}>
            ...
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
              <FormControl fullWidth disabled={Boolean(form.taskId)}>
                <InputLabel id="status-label">Status</InputLabel>
                <Select
                  labelId="status-label"
                  name="status"
            ...
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
              ...
              <TextField
                ...
                fullWidth
                disabled={Boolean(form.taskId)}
                helperText={form.taskId ? "Due date is taken from the linked task" : ""}
              />
            </Stack>
