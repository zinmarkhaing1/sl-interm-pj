...
export const NewNotePage: React.FC = () => {
  ...
  // ---- Fetch existing tasks (to link note to a task) ----
  const { data: tasks, isError: tasksError } = useGetTasksQuery({});
  const { data: existingNotes = [] } = useGetNotesQuery();
  ...
  // ---- Initialize from taskContext ----
  useEffect(() => {
    if (!taskContext?.taskId) return;
    setForm((prev) => ({
      ...prev,
      taskId: taskContext.taskId || prev.taskId,
      assignee: taskContext.assignee || prev.assignee,
    }));
  }, [taskContext?.taskId, taskContext?.taskTitle, taskContext?.assignee]);

  // ---- Fetch selected task details ----
  const selectedTaskId = form.taskId;
  const { data: selectedTask } = useGetTaskQuery(selectedTaskId, {
    skip: !selectedTaskId,
  });

  // ---- Helper to get assignee name from task ----
  const getTaskAssignee = (
    task: NonNullable<typeof tasks>[number] | undefined,
  ) => {
    if (!task?.assignee) return "";
  ...
  // ---- Helper to get project name from task (for category) ----
  ...
  useEffect(() => {
    ...
    setForm((prev) => ({
      ...
      category: getTaskProjectName(selectedTask) || prev.category,

      priority: selectedTask.priority || prev.priority,

      status: selectedTask.status || prev.status,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const linkedTask =
        selectedTask || (tasks || []).find((task) => task._id === form.taskId);
      const linkedAssignee =
        getTaskAssignee(linkedTask) || taskContext?.assignee || "";
      const payload: any = {
        title: form.title.trim(),
        content: form.description.trim() || " ",
        description: form.description.trim(),
        ...
        priority: form.priority,
        task: form.status,
        assignee: form.taskId
          ? linkedAssignee || "Unassigned"
      ...
      if (form.taskId) {
        payload.taskId = form.taskId;
        payload.taskTitle =
          linkedTask?.title || taskContext?.taskTitle || form.title.trim();
      }
  ...
  return (
    <Box sx={{ maxWidth: 640, mx: "auto", width: "100%", py: 1 }}>
      ...
      >
        ...
        {tasksError && (
          <Alert severity="info" sx={{ mb: 2 }}>
            Existing tasks could not be loaded, but you can still create a note
            without linking a task.
          </Alert>
        )}
        ...
        <form onSubmit={handleSubmit}>
          <Stack spacing={2.5}>
            ...
            {/* Category – locked when task is linked */}
            {/* <FormControl fullWidth required disabled={Boolean(form.taskId)}>
              <InputLabel id="category-select-label">Category</InputLabel>
              <Select
                labelId="category-select-label"
                label="Category"
              ...
              <FormHelperText>
                {form.taskId ? "Category is taken from the linked task's project" : "Select a category for this note."}
              </FormHelperText>
            </FormControl> */}

            {/* Category – Read-only when task is linked, dropdown otherwise */}
            {form.taskId ? (
              <TextField
                fullWidth
                label="Category"
                ...
                // disabled
                helperText="Category is taken from the linked task's project"
              />
            ) : (
              <FormControl fullWidth required>
                <InputLabel id="category-select-label">Category</InputLabel>
            ...
            {/* Assignee – locked when task is linked */}
            <TextField
              label={form.taskId ? "Task assignee" : "Assignee"}
              name="assignee"
              value={form.assignee}
              ...
              helperText={
                form.taskId
                  ? "Taken from the selected task"
                  : "Who is responsible for this note?"
              }
              // disabled={Boolean(form.taskId)}
            />

            {/* Status & Priority – both locked when task is linked */}
            <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
              <FormControl fullWidth >
                <InputLabel id="status-label">Status</InputLabel>
                <Select
                ...
                <FormHelperText>
                  {form.taskId ? "Status is taken from the linked task" : ""}
                </FormHelperText>
              </FormControl>
              ...
              <FormControl fullWidth > 
                ...
                <FormHelperText>
                  {form.taskId ? "Priority is taken from the linked task" : ""}
                </FormHelperText>
              </FormControl>
            </Stack>

            {/* Start & End Dates – endDate is locked when task is linked (due date) */}
            <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
              <TextField
                label="Start date"
                type="date"
              ...
              <TextField
                ...
                // disabled={Boolean(form.taskId)}
                helperText={
                  form.taskId ? "Due date is taken from the linked task" : ""
                }
              />
            </Stack>
