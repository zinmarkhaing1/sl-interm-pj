...
// console.log("noteId:", note._id.toString());
// console.log("sharedAccess:", sharedAccess);
// console.log("permission:", sharedAccess?.permission);

//   if (sharedAccess && ["comment", "edit", "full"].includes(sharedAccess.permission)) {
...
//     const { hasAccess, note, isOwner,sharedAccess } = await checkNoteAccess(
...
//     if (isOwner) {
//       // Owner sees ALL comments from ALL collaborators
//       comments = await Comment.find({ noteId: note._id })
//         .sort({ createdAt: 1 })
//         .lean();
//     } else {
//       // Collaborator sees only their own comments + owner's comments
//       const ownerId = note.user.toString();
//       const conversationId = generateConversationId(noteId as string, userId, ownerId);
...
//     // Generate conversation ID for private comments
//     if (!isOwner) {
...
//       }
//     } else {
//       // Owner commenting - if replying to specific collaborator
//       const replyToUserId = req.body.replyToUserId;
//       if (replyToUserId) {
//         conversationId = generateConversationId(noteId as string , userId, replyToUserId);
//       }
//     }
...
//     // Only owner can delete comments of removed user
//     const isOwner = note.user.toString() === userId;
//     if (!isOwner) {
...
//     // Delete all comments by this user on this note
